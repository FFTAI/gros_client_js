...
export class Motor extends RobotBase {
   ...
   */
  public async set_motor_pd_flag(no: string, orientation: string) {
    const data = {
      no: no,
   ...
   * @param {String} orientation  Motor orientation.
   */
  public async enable_motor(no: string, orientation: string) {
    if (parseInt(no) > 8) {
      console.log(
   ...
   * @param {String} orientation  Motor orientation.
   */
  public async disable_motor(no: string, orientation: string) {
    if (parseInt(no) > 8) {
      console.log(
        `Motor enabled fail: This function can only control 0-8. But current value: ${no}`
      );
   ...
   */
  public async move_motor(no: string, orientation: string, angle: number) {
    const motors: any = [];
    motors.push({
      no: no,
      orientation: orientation,
   ...
   * @param {String} orientation  Motor orientation.
   */
  public async get_motor_pvc(no: string, orientation: string) {
    const data = {
        no: no,
        orientation: orientation,
      };
   ...
   *
   */
  public async get_hand_position() {
    return super.http_request({
      method: "POST",
